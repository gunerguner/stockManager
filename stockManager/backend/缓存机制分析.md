...
| `StockNameSync` 有名称变更并 `bulk_update` 后 | `clear_stock_meta_all` + `mark_stock_name_synced_today` |
...
- 今日已同步 → 跳过
- 无名称变更 → 仅 `mark_stock_name_synced_today`
- 有变更 → `bulk_update` → `clear_stock_meta_all` → 标记今日已同步
