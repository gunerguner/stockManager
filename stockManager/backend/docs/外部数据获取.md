...
- **A 股**：`use('tencent').real(codes, prefix=True)` → `currentPrice`、`yesterdayClose`、`yearHigh` 等。
- **港股**：`use('hkquote').real([5位代码])`。
...
- **失败**：`fx_store` 非交易时段可回落 Redis 已有 `fx:hkd_cny`。

## 4. 调用路径

| API / 功能 | 入口 | 外部数据 |
|-----------|------|---------|
| `GET /api/stocks` | `Integrate.get_calculated_result` | 实时价、汇率（含港股时） |
