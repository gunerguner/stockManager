...
| 业务  | `integrate.py`、`market/`、`calculation/`、`dividend.py` | 经 `CacheRepository` 使用缓存 |
...
| `user_store.py` | 用户 operations、cash_info、calculated_target 读写与失效 |
| `price_store.py` | 股价批量读写、timestamp、交易时段刷新策略 |
| `meta_store.py` | StockMeta 全量缓存、名称日同步标记 |
| `repository.py` | `CacheRepository` 入口 |
...
| `market/` | `from services.market import RealtimePrice, StockMeta` |
...
3. **用户计算结果（聚合）**
  ...
  - 用途：`/api/stocks` 直接返回，避免重复计算；无 `markets` 字段的旧缓存视为失效
4. **股票元数据全量字典**
  - Key：`stock:meta:all`
...
6. **股票价格时间戳（分市场）**
  ...
  - 内容：上海时区 ISO 时间字符串
  - 用途：按 CN/HK 独立判断价格缓存是否“逻辑过期”；旧 key `stock:price:timestamp` 首次读 CN 时迁移
7. **港币汇率**
  - Key：`fx:hkd_cny`
  - 内容：`float`（1 HKD = X CNY）
  - 用途：港股通 Overall 汇总折算；TTL 24h 物理兜底，收盘后随 `calculated_target` 冻结，不因汇率 alone 失效
8. **股票名称日同步标记**
...
核心：`price_store._should_refresh_market(market, relevant=...)`，按用户持仓涉及的市场判断。
...
- `get_stock_prices_with_cache`：仅对本批 `code_list` 中出现的市场做上述判断；CN/HK 独立。

相关行为：

- `get_calculated_target(user, user_codes)`：持仓涉及任市场需刷新则返回 `None`
- `set_calculated_target`：仅当**持仓涉及的所有市场**均非交易时段才写入
...
| 港币汇率  | `fx:hkd_cny`                  | 24h           | Integrate 重算时读取      | `clear_all()`；交易时段重算拉 API 更新              |
| 名称日同步 | `stock:name:sync:daily`       | 至次日 00:00+60s | 控制每日最多同步一次          | 次日过期                                    |


## 10. 新增缓存时的检查清单

1. 在 `backend/services/cache/keys.py` 增加逻辑 key 与 `TTL_*`（如需）；对外 API 经 `facade.CacheRepository` 暴露。
...
- 缓存仓库：`backend/services/cache/`（`repository.py`）
- Redis 工具：`backend/common/cache.py`
- 交易时段：`backend/common/tradingCalendar.py`
- 用户数据与信号：`backend/services/integrate.py`
- 实时行情：`backend/services/market/realtimePrice.py`
- 名称同步：`backend/services/market/stockNameSync.py`
- 元数据：`backend/services/market/stockMeta.py`
