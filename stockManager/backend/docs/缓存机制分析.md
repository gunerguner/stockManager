...
flowchart LR
  ...
  subgraph market [market 数据源]
    ...
    fetchVal[fetch_cn_valuation / fetch_pe_pb]
    fetchHist[fetch_cn_hist_highs / fetch_hk_hist_high]
  end
...
| `refresh_policy.py` | 交易时段判断、价格时间戳读写、`should_refresh_market` |
...
| `market/` | `from services.market import fetch_prices, fetch_hkd_cny_rate, fetch_cn_valuation, fetch_pe_pb, fetch_cn_hist_highs, fetch_hk_hist_high` |
...
10. **单股估值指标**
  ...
  - 用途：关注列表 PE/PB 计算；A 股走 baostock，港股走百度 opendata
11. **单股历史最高价**
  - Key：`stock:hist_high:{code}`
  - 内容：`float` 或哨兵值 `__none__`（表示 API 无数据，避免反复回源）
  - 用途：关注列表 histHigh；A 股走 baostock，港股走 gtimg
...
| `TTL_VALUATION` | 86400 | 单股估值，24 小时 |
| `TTL_HIST_HIGH` | 86400 | 单股历史高价，24 小时 |
...
核心：`refresh_policy.should_refresh_market(market)`，按本批代码涉及的市场独立判断。

- **当前在该市场交易时段**（`TradingCalendar.is_current_time_in_trading_hours(market)`）→ 该市场价格视为需刷新。
- **不在该市场交易时段**：读 `stock:price:timestamp:{market}`，无时间戳或 `is_trading_time_passed(..., market)` 则刷新。
- `price_store._get_cached_prices`：仅对本批 `code_list` 中出现的市场做上述判断；CN/HK 独立。

相关行为：

- `user_store.get_calculated_target(user, user_codes)`：持仓涉及任市场需刷新则返回 `None`
- `user_store.set_calculated_target`：仅当**持仓涉及的所有市场**均非交易时段才写入
- `fx_store.get_hkd_cny_rate`：持仓涉及任市场在交易时段则跳过读缓存、强制回源
- 纯 A 股用户不受港股 15:00–16:00 时段影响
...
5. `set_calculated_target`（交易时段内跳过写入）
...
2. `CacheRepository.load_watchlist_market_data(codes)` 聚合：
   - `price_store.query_prices`（共用 §6.2 逻辑失效）
   - `valuation_store.get_valuations`（miss 回源 baostock / 百度）
   - `hist_high_store.get_hist_highs`（miss 回源 baostock / gtimg）
3. 与持仓状态、用户配置合并后返回
...
| 计算结果 | `user:{id}:calculated_target` | 24h | 持仓涉及市场需刷新则失效；否则命中 | 用户数据变更；分市场价格时间戳更新后 pattern 清全用户 |
...
| 实时价格 | `stock:price:{code}` | 24h | 批量 MGET + 缺失回源 API | §5.1 按市场逻辑失效；TTL 自然过期 |
| 价格时间戳 | `stock:price:timestamp:cn` / `:hk` | 24h | 供分市场逻辑失效判断 | 批量写价时仅更新涉及市场的时间戳 |
| 港币汇率 | `fx:hkd_cny` | 24h | 非交易时段读缓存；交易时段回源 | `clear_all()`；交易时段强制 API 更新 |
| 名称日同步 | `stock:name:sync:mark` | 24h | 控制 24h 内最多同步一次 | TTL 自然过期 |
| 单股估值 | `stock:valuation:{code}` | 24h | miss 回源 baostock / 百度 | TTL 自然过期；`clear_all()` |
| 单股历史高价 | `stock:hist_high:{code}` | 24h | miss 回源 baostock / gtimg | TTL 自然过期；`clear_all()` |
...
- 估值：`backend/services/market/baostock_source.py`、`baiduValuation.py`
