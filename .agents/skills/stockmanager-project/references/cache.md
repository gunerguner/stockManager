...
8. **港币汇率**
  ...
  - 用途：港股通市值与盈亏折算；非交易时段优先读缓存，持仓涉及任一市场处于交易时段时强制回源 API
9. **股票名称日同步标记**
...
11. **单股历史最高价**
  ...
  - 用途：关注列表 histHigh；A 股与港股均走 gtimg 周线前复权（qfq）
...
| 单股历史高价 | `stock:hist_high:{code}` | 30 天 | miss 回源 gtimg 周线（qfq） | TTL 自然过期；`clear_all()` |
